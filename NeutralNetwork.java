...
public class NeutralNetwork {
    ...
    public void generateInput(){
        ...
        for(int j=0;j<ntrain;j++)
            for (int k=0;k<dim;k++)
                input[j].add((double)getGaussian(1,0.1));
    ...
    public void generateTestdata(){
        ...
        for(int j=0;j<ntest;j++)
            for (int k=0;k<dim;k++)
                testdata[j].add((double)getGaussian(1,0.1));
    }
    ...
    public double neuralLearning(int itime,double lan, double lrate) {
        ...
        for (int k = 0; k < iteration; k++) {
            ...
            for (int ee = 0; ee < ntrain; ee++)
                trainError += sum[ee];
            /** System.out.println(trainerror+"the first time");*/
            double wsum1 = 0;
            for (int y = 0; y < hid; y++)
                for (int z = 0; z < dim; z++)
    ...
    public void feedForward(int ntrain, Vector [] input){
           ...
           /** System.out.println(aj[0].elementAt(1));*/
            ...
            /**for(int hh=0;hh<ntrain;hh++)
                for(int dd=0;dd<hid;dd++)
                     System.out.println((double)aj[hh].elementAt(dd)+"hidden hh dd");*/
            ...
            for(int nn=0;nn<ntrain;nn++)
                for(int ii=0;ii<dim;ii++) {
                    ...
                    /**System.out.println((double)ak[nn].elementAt(ii)+"ak nn ii");*/

                }

            /**System.out.println(ak[1].elementAt(2)+"this is test");*/
    ...
    public void writeFile(double [][] array,String filename) throws IOException {
        BufferedWriter bw= new BufferedWriter(new FileWriter("/Users/yujingxie/documents/"+filename+".txt"));
