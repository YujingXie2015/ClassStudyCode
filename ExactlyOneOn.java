...
public class ExactlyOneOn {
    ...
    public static void main(String [] args) throws IOException{
        ...
        for(int i=0;i<10;i++) {
            ...
            /**if(i==5) {
                neutral.writeFile(neutral.w1, "w11");
                neutral.writeFile(neutral.w2, "w22");
            }*/
        }
        double [] train=new double[2];
        train=NeutralNetwork.meanDeviation(trainError);
        DecimalFormat df = new DecimalFormat("0.000");
        System.out.println(df.format(train[0]*100) + "±" + df.format(train[1]*100));
        double [] test=new double[2];
        test=NeutralNetwork.meanDeviation(testError);
        System.out.println(df.format(test[0]*100) + "±" + df.format(test[1]*100));
