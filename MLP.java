...
public class MLP implements Serializable {
    ...
    private void writeObject(ObjectOutputStream outputStream) throws IOException{
        ...
        outputStream.writeInt(nInputs);
        outputStream.writeInt(nHidden);
        outputStream.writeInt(nOutput);
        
        for(int j=1; j<=nHidden; j++)
            for(int i=0; i<=nInputs; i++) {
                outputStream.writeDouble(weightL1[j][i]);
            }
        
        for(int j=1; j<=nOutput; j++)
            for(int i=0; i<=nHidden; i++) {
                outputStream.writeDouble(weigthL2[j][i]);
            }
    }
    
    private void readObject(ObjectInputStream inputStream) throws IOException,ClassNotFoundException{
        inputStream.defaultReadObject();
        this.nInputs = inputStream.readInt();
        this.nHidden = inputStream.readInt();
        this.nOutput = inputStream.readInt();
        
        input = new double[this.nInputs+1];
        hidden = new double[this.nHidden+1];
        output = new double[this.nOutput+1];
        
        weightL1 = new double[this.nHidden+1][this.nInputs+1];
        weigthL2 = new double[this.nOutput+1][this.nHidden+1];
        
        for(int j=1; j<=this.nHidden; j++)
            for(int i=0; i<=this.nInputs; i++) {
                weightL1[j][i] = inputStream.readDouble();
            }
        
        for(int j=1; j<=this.nOutput; j++)
            for(int i=0; i<=this.nHidden; i++) {
                weigthL2[j][i] = inputStream.readDouble();
            }
    }
